...
const entries: Reducer<Array<Entry[]>, ActionTypes> = (state = initialState.entries, action: ActionTypes): Array<Entry[]> => {
    switch (action.type) {
        case ActionTypeKeys.FETCH_ENTRIES_RESOLVED:
            const words = action.payload.data.sort((a: Entry[], b: Entry[]) => a[0].entry_id > b[0].entry_id ? 1 : a[0].entry_id < b[0].entry_id ? -1 : 0);
            return words.map((w: any) => w.sort((a: Entry, b: Entry) => a.entry_id > b.entry_id ? 1 : a.entry_id < b.entry_id ? -1 : 0));
